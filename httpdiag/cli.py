...
def print_result(result, show_body=True):
    ...
    if len(result.headers) > 10:
        print(f"    ... 还有 {len(result.headers) - 10} 个响应头")
    print()
    if show_body:
        print(f"  响应体 (前200字符):")
        print(f"    {repr(result.body_preview)}")
        print(f"  响应体总长度: {len(result.body)} 字节")
...
示例:
  ...
  %(prog)s --no-redirect https://www.example.com
    ...
    if not headers:
        headers["User-Agent"] = "httpdiag/1.0"
        headers["Accept"] = "*/*"
    ...
    if args.compare:
        ...
        if args.log_file:
            ...
            filepath = logger.save_single_request(
                compare_result.items[0].result,
                filename=args.log_file + "_1" if args.log_file else None,
            )
            filepath2 = logger.save_single_request(
                compare_result.items[1].result,
                filename=args.log_file + "_2" if args.log_file else None,
            )
            print(f"  日志已保存到:")
            print(f"    {filepath}")
            print(f"    {filepath2}")
